...
function randomCol(){
	let p = null;
	if (typeof paletteSelected !== "undefined" && paletteSelected) p = paletteSelected;
	else if (typeof paletteSelected1 !== "undefined" && paletteSelected1) p = paletteSelected1;
	else if (typeof paletteSelected2 !== "undefined" && paletteSelected2) p = paletteSelected2;
	else p = palettes[0];
	return typeof random === "function" ? random(p) : p[Math.floor(Math.random() * p.length)];
 }
function bgCol(){
	if (typeof paletteSelected !== "undefined" && paletteSelected && paletteSelected.length > 0) {
		return paletteSelected[0];
	}
	return palettes[0][0];
 }
...
// 回傳背景色（會使用 sketch.js 中的 paletteSelected，如果尚未設定則回傳預設）
function bgCol() {
  if (typeof paletteSelected !== "undefined" && paletteSelected && paletteSelected.length > 0) {
    return paletteSelected[0];
  }
...
// 隨機從可用色票中取一色（優先使用 paletteSelected、paletteSelected1、paletteSelected2）
function randomCol() {
  let p = null;
  if (typeof paletteSelected !== "undefined" && paletteSelected) p = paletteSelected;
  else if (typeof paletteSelected1 !== "undefined" && paletteSelected1) p = paletteSelected1;
